const CodeStormApplicant25 = require("../models/codestormapplicant.model");
const {
  checkName,
  checkEmail,
  checkPhone,
...
exports.sendcodestormuser = async (req, res) => {
  try {
    ...
    const applicant = new CodeStormApplicant25({
      ...rec.payload,
    });
...
exports.VerifyCodeStorm = async (req, res) => {
  ...
  const results = await CodeStormApplicant25.find({ id: req.body.payload });
  if (results.length == 0) {
    res.status(422).json([
      {
...
exports.exportCodeStorm = async (req, res) => {
  ...
  const opts = { fields };
  const results = await CodeStormApplicant25.find({});
