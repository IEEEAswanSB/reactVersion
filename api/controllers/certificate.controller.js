...
exports.importCertificates = async (req, res) => {
  ...
  for (var i = 0; i < themes.length; i++) ThemesCache.add(themes[i]["theme"]);

  for (var i = 0; i < IDs.length; i++) DBIdCache.add(IDs[i]["id"]);
  ...
  Data.pipe(parse({ delimiter: ",", from_line: 2 }))
    .on("data", function (row) {
      ...
      for (var i = 2; i < row.length - 1; i++) {
        if (row[i] != undefined && row[i] != "") AdditionalTexts.push(row[i]);
        else {
          err = 1;
          res.status(200).json({
            err: 1,
            msg:
              "An attribute at row " +
              CurrentRow +
              " is undefined. No certificate is inserted",
          });
      ...
      if (!ThemesCache.has(CertificateData["theme"])) {
        ...
        res.status(200).json({
          ...
          msg:
            ...
            " at row " +
            CurrentRow +
            " cannot be found. No certificate is inserted",
        });
      ...
      if (DBIdCache.has(CertificateData["id"])) {
        ...
        res.status(200).json({
          ...
          msg:
            ...
            " at row " +
            CurrentRow +
            " already exists. No certificate is inserted",
        });
      ...
      if (CSVIdCache.has(CertificateData["id"])) {
        ...
        res.status(200).json({
          ...
          msg:
            ...
            " at row " +
            CurrentRow +
            " is found 2 times or more in the CSV file. No certificate is inserted",
        });
    ...
    .on("end", async function () {
      ...
      try {
        ...
        res.status(200).json({
          err: 1,
          msg: "An Unidentified Error Occurred. Some certificates may have been inserted",
        });
        res.end();
        return;
      }
      ...
      if (err == 0) {
        res.status(200).json({
          err: 0,
          msg: "All certificates have been inserted successfully.",
        });
        res.end();
      }
    });
...
exports.ExtractPDF = async (req, res) => {
  ...
  const results = await Certificate.find({
    ...
    name: req.body["payload"]["payload"]["name"],
  });
  if (results.length == 0) {
    res.status(400);
    res.end();
  ...
  for (
    var i = 0;
    i <= req.body["payload"]["payload"]["additionalTexts"].length;
    i++
  ) {
    ...
    const Text2Write =
      i == 0
        ? req.body["payload"]["payload"]["name"]
        : req.body["payload"]["payload"]["additionalTexts"][i - 1];
