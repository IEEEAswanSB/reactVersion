...
const fs = require("fs");
const {checkName,checkLatinName,checkEmail,
    checkPhone,checkNationalId,checkImg} = require('../Helpers/Validators')
...
try{
    ...
    let {data:captchaRes}  =  await captchaVerify.post('', null, {
        params: {
            secret: '6Lci-F4pAAAAAAki_zSJpZscx_dCSanl2D3PxlfQ',
            response: Captcha
        }
    });
    ...
    if(checkLatinName(certificateName)){
        return res.status(400).json({message: 'Please enter a valid certificate name'});
    }
    ...
    if(checkNationalId(id)){
        return res.status(400).json({message: 'Please enter a valid national ID'});
    }
    ...
    const convertedImage = await sharp(paymentImage.data).toFormat('webp').toBuffer();
...
exports.beincamp6Validate = async (req, res) => {
    try{
        ...
        if(!TicketID){
            return res.status(400).json({message: 'Please enter a valid Ticket ID'});
        }
        ...
        if(mongoRes.validated){
            return res.status(400).json({message: 'This user is already validated'});
        }
...
exports.beincamp6recordAttendance = async (req, res) => {
    try{
        ...
        if(!mongoRes.validated){
            return res.status(400).json({message: `${mongoRes.name} is not validated`});
        }

        let Days = [1706486400000,1706572800000,1706659200000,1706745600000,1706832000000]

        let today = new Date().getTime();

        let todayIndex = Days.findIndex((day) => today >= day && today < day + 86400000);
        ...
        if(mongoRes[day]){
            return res.status(400).json({message: `${mongoRes.name} is already attended today`});
        }
        ...
        return res.status(200).json({message: `${mongoRes.name} Attendance recorded Successfully`});
    ...
    sheetsAPI.spreadsheets.values.batchUpdate ({
      ...
      const checkboxFormat = {
        repeatCell:
        {
            ...
            range: {sheetId: 0, startRowIndex: 1, endRowIndex: results.length +1,
                      startColumnIndex: 15,
                      endColumnIndex: 16},
