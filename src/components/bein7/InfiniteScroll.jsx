...
export default function InfiniteScroll({
  ...
  negativeMargin = "-20.5em",   // Negative margin to reduce spacing between items
  // ----- Items Prop -----
  items = [],                  // Array of items with { content: ... }
  itemMinHeight = 300,         // Fixed height for each item
  // ----- Tilt Props -----
  ...
  useEffect(() => {
    ...
    const observer = Observer.create({
      ...
      onChange: ({ deltaY, isDragging, event }) => {
        ...
        const distance = isDragging ? d * 10 : d * 10;
  ...
  return (
    ...
    >
      ...
      >
        {items.map((item, i) => (
          <div
            className="flex items-center justify-center text-xl font-semibold text-center border-2 border-white rounded-[15px] select-none box-border relative"
            key={i}
            style={{
