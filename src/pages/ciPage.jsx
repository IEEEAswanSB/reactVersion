...
import { ExportCodeStormTicket,validateCertificate,ExtractPDF } from "../../services/register.service.js";
let global = {};
function Errmessage() {
...
function CiCodeStormForm() {
  ...
  let download = async()=>{
    ...
    await validateCertificate(payload).then((res)=>{
    ...
    ExtractPDF(SentPayload).then(res=>{
        downloadBase64File(res,"Certificate "+payload.name + '-' + id +".pdf");
    })
  ...
  return (
    <>
      ...
      <div className="codestorm-page  absolute top-0 left-0 w-screen h-screen overflow-hidden ">
        ...
        <Errmessage/>
          <h1 className="text-slate-200 sm:text-2xl p-4 rounded-lg  border-2 border-slate-200  ">Enter your certificate ID</h1>
