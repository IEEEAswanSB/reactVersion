...
function VerifyCodeStorm(props) {
  ...
  let [error,setError] = useState(false)
   ...
   useEffect(()=>{
    ...
    VerifyCodeStormService(id).then((res)=>{
    ...
    <>
      <div className="codestorm-page  absolute top-0 left-0 w-screen h-screen overflow-hidden ">
        <div className="flex justify-center items-center h-screen overflow-hidden">
          {
           error&& <h1 className="text-slate-200 sm:text-2xl p-4 bg-red-900 rounded-lg  border-2 border-slate-200 ">{msg}</h1>
          }
          {
           done&& <h1 className="text-slate-200 sm:text-2xl p-4 bg-green-900 rounded-lg  border-2 border-slate-200 ">{msg}</h1>
          }
